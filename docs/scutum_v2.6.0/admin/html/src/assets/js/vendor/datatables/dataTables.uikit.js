...
(function (factory) {
	...
	var dom = isRTL()
		? "<'row uk-grid uk-flex-middle uk-grid-small dt-uikit-header'<'uk-width-1-2@m'l><'uk-width-1-2@m uk-text-left@m'f>>" +
			...
			for (i = 0, ien = buttons.length; i < ien; i++) {
				...
				if ($.isArray(button)) {
					...
					case 'ellipsis':
						...
						case 'first':
						var fIcon = isRTL() ? 'mdi-chevron-double-right' : 'mdi-chevron-double-left'
						btnDisplay = '<i class="mdi ' + fIcon + '"></i> ' + lang.sFirst;
						btnClass = (page > 0
							? ' uk-pagination-first' : ' uk-pagination-first uk-disabled disabled');
						break;

					case 'previous':
						var pIcon = isRTL() ? 'mdi-chevron-right' : 'mdi-chevron-left'
						btnDisplay = '<i class="mdi ' + pIcon + '"></i> ' + lang.sPrevious;
						btnClass = (page > 0
							? ' uk-pagination-prev' : ' uk-pagination-prev uk-disabled disabled');
						break;

					case 'next':
						var nIcon = isRTL() ? 'mdi-chevron-left' : 'mdi-chevron-right'
						btnDisplay = lang.sNext + ' <i class="mdi ' + nIcon + '"></i>';
						btnClass = (page < pages - 1
							? ' uk-pagination-next' : ' uk-pagination-next uk-disabled disabled');
						break;

					case 'last':
						var lIcon = isRTL() ? 'mdi-chevron-double-left' : 'mdi-chevron-double-right'
						btnDisplay = lang.sLast + ' <i class="mdi ' + lIcon + '"></i>';
						btnClass = (page < pages - 1
