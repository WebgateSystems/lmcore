...
module.exports = laravelNuxt({
	...
	*/
	head: {
		...
		script: [
			{ src: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist ) + '/vendor/uikit.min.js'}
		],
		link: [
			{ rel: 'icon', type: 'image/x-icon', href: (process.env.NODE_ENV !== 'production' ? '' :  '/' + dist ) + '/favicon.ico'},
			{ rel: 'preload', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/vendor/uikit.min.js', as: 'script' },
			{ rel: 'preload', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/fonts/roboto_base64.css', as: 'style' },
			{ rel: 'preload', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/fonts/sourceCodePro_base64.css', as: 'style' },
			{ rel: 'preload', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/fonts/mdi/css/materialdesignicons.css', as: 'style' },
			{ rel: 'stylesheet', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/fonts/roboto_base64.css' },
			{ rel: 'stylesheet', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/fonts/sourceCodePro_base64.css' },
			{ rel: 'stylesheet', href: (process.env.NODE_ENV !== 'production' ? '' : '/' + dist) + '/fonts/mdi/css/materialdesignicons.css' }
		]
	},
	/*
	...
	build: {
		...
		babel: {
			...
			ignore: [
				"node_modules",
				"assets/js/vendor",
				"resources/nuxt/assets/js/vendor"
			]
		},
		extend (config, ctx) {
			...
			// aliases
			config.resolve.alias['scss'] = path.resolve(__dirname, './resources/nuxt/assets/scss');
			// adjust path when serving site from subfolder
			if (!ctx.isDev) {
