		...
		var attach = function (container, buttons) {
			...
			for (i = 0, ien = buttons.length; i < ien; i++) {
				...
				if ($.isArray(button)) {
					...
					case 'first':
						btnDisplay = '<i class="mdi mdi-chevron-double-right"></i> ' + lang.sFirst;
						btnClass = (page > 0
							? ' uk-pagination-first' : ' uk-pagination-first uk-disabled disabled');
						break;

					case 'previous':
						btnDisplay = '<i class="mdi mdi-chevron-right"></i> ' + lang.sPrevious;
						btnClass = (page > 0
							? ' uk-pagination-prev' : ' uk-pagination-prev uk-disabled disabled');
						break;

					case 'next':
						btnDisplay = lang.sNext + ' <i class="mdi mdi-chevron-left"></i>';
						btnClass = (page < pages - 1
							? ' uk-pagination-next' : ' uk-pagination-next uk-disabled disabled');
						break;

					case 'last':
						btnDisplay = lang.sLast + ' <i class="mdi mdi-chevron-double-left"></i>';
						btnClass = (page < pages - 1
