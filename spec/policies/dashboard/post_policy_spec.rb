...
RSpec.describe Dashboard::PostPolicy, type: :policy do
  ...
  # The /dashboard area is intentionally siloed per-blog. A moderator role
  # does NOT grant cross-blog access here -- platform-wide moderation lives
  # under /admin instead.
  context "when user is a moderator on someone else's post" do
    let(:user) { moderator }
  ...
  describe Dashboard::PostPolicy::Scope do
    ...
    it "limits moderators to their own posts too (dashboard is per-blog)" do
      moderator_post = create(:post, author: moderator)
      create(:post, author: owner)
      create(:post, author: other_author)

      scope = described_class.new(moderator, Post.all).resolve
      expect(scope).to contain_exactly(moderator_post)
    end
  end
end
